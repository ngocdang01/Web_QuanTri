...
const SaleProducts = () => {
  ...
  const sizes = Array.isArray(product.sizes)
    ? product.sizes
    : Array.isArray(product.size_items)
      ? product.size_items
      : [];
  ...
  return {
    ...
    images: Array.isArray(product.images) ? product.images : []
      ...
      console.log("Error load sale products:", error);
  ...
  const handleSubmit = async(e) => {
    ...
    const stock = newSaleProduct.size_items.reduce((acc, item) => acc + item.quantity, 0);
    ...
    if (size_items.length === 0 || size_items.every(s => s.quantity <= 0)) {
    ...
    // Validate stock > 0
    if (stock <= 0) { alert("Tổng số lượng tồn kho phải lớn hơn 0");
      return;
    }
  ...
  const sizes = updatedProduct.sizes ?? updatedProduct.size_items ?? [];
  ...
  console.log("API Update Error:", error.response?.data);
                      ...
                      <td>
                        {Array.isArray(product.sizes) && product.sizes.length > 0
                          ...
                          : Array.isArray(product.size_items) && product.size_items.length > 0
                            ? product.size_items.map(s => `${s.size} (${s.quantity})`).join(", ")
                            : "N/A"
                        }
